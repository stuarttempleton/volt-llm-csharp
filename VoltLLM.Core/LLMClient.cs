...
public class LLMClient
{
    ...
    /// <param name="temperature">The temperature setting for responses (default is 0.2).</param>
        public LLMClient(string baseUrl = "http://localhost:11434/", string? token = null, string model = "Gemma3", float temperature = 0.2f)
        {
            this.baseUrl = baseUrl.TrimEnd('/');
            this.model = model;
            this.temperature = temperature;
            this.bearerToken = token ?? Environment.GetEnvironmentVariable("LLM_API_TOKEN") ?? "";
            this.client = new HttpClient();
            this.DetectApiTypeAsync().Wait();
        }
    ...
    /// <returns>The response content from the LLM API, or null if the request fails.</returns>
        public async Task<string?> SendPromptAsync(string prompt, string? systemPrompt = null)
        {
            var messages = new[]
            {
                new Dictionary<string, string> { ["role"] = "system", ["content"] = systemPrompt ?? "You are a senior application security engineer." },

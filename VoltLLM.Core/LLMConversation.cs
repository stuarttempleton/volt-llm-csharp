...
namespace VoltLLM.Core
{
    ...
    public LLMConversation(string model = "gemma3", string systemPrompt = "", string token = "", string baseUrl = "http://localhost:11434/")
    {
        ...
        messages = new List<Dictionary<string, string>>
        {
            ...
            {
                { "role", "system" },
                { "content", string.IsNullOrEmpty(systemPrompt) ? "You are a helpful and knowledgeable AI assistant. Provide clear, concise, and accurate responses. When appropriate, ask clarifying questions or provide examples." : systemPrompt }
            }
        };
    }
